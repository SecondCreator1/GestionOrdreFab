...
export class OrdreFabricationEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<any[]>('http://localhost:8080/api/produits')
      .subscribe(
        (data) => {
          this.produits = data;
        },
  ...
  loadOrdre(id: number): void {
    this.http.get<any>(`http://localhost:8080/api/ordres/${id}`)
      .subscribe(
        (ordre) => {
  ...
  onSubmit(): void {
    if (this.ordreForm.valid) {
      ...
      this.http.put(`http://localhost:8080/api/ordres/${this.ordreId}`, updatedOrdre)
        .subscribe(
          () => {
